    ...
    switch ($resource) {
        ...
        case 'segments':
            ...
            elseif ($method === 'POST' && empty($id)) {
                ...
                $stmt = $db->prepare('INSERT INTO segments (name, slug, short_description, full_description, rules, eligibility, min_team_size, max_team_size, registration_fee, fee_additional, fee_note, prize_pool, prize_details, google_form_url, is_active, display_order, image_path, rulebook_path) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
                $stmt->execute([$f['name'], $f['slug'] ?: strtolower(preg_replace('/[^a-z0-9]+/i', '-', $f['name'])), $f['short_description'], $f['full_description'] ?? '', $f['rules'] ?? '', $f['eligibility'] ?? '', (int)($f['min_team_size'] ?? 1), (int)($f['max_team_size'] ?? 5), (float)($f['registration_fee'] ?? 0), (float)($f['fee_additional'] ?? 0), $f['fee_note'] ?? '', $f['prize_pool'] ?? '', $f['prize_details'] ?? '', $f['google_form_url'] ?? '', (int)($f['is_active'] ?? 1), (int)($f['display_order'] ?? 0), $img, $rb]);
            ...
            elseif ($method === 'POST' && !empty($id)) {
                ...
                if (!$ex) error('Segment not found.', 404);
                ...
                $stmt = $db->prepare('UPDATE segments SET name=?, slug=?, short_description=?, full_description=?, rules=?, eligibility=?, min_team_size=?, max_team_size=?, registration_fee=?, fee_additional=?, fee_note=?, prize_pool=?, prize_details=?, google_form_url=?, is_active=?, display_order=?, image_path=?, rulebook_path=?, updated_at=NOW() WHERE id=?');
                $stmt->execute([$f['name'], $f['slug'], $f['short_description'], $f['full_description'] ?? '', $f['rules'] ?? '', $f['eligibility'] ?? '', (int)($f['min_team_size'] ?? 1), (int)($f['max_team_size'] ?? 5), (float)($f['registration_fee'] ?? 0), (float)($f['fee_additional'] ?? 0), $f['fee_note'] ?? '', $f['prize_pool'] ?? '', $f['prize_details'] ?? '', $f['google_form_url'] ?? '', (int)($f['is_active'] ?? 1), (int)($f['display_order'] ?? 0), $img, $rb, $id]);
